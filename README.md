...
- __saveplaylist__(p)
  - Saves the current playlist to m3u file. Saves to `mpv/playlists/` by default. There is an experimental setting `interactive_playlist_save` that allows you to type in the filename instead of having it autogenerated, this requires you to use `playlistmanager-save-interactive.lua` script.
